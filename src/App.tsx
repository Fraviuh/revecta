        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
          ...
          {advantages.map((a) => (
            <Card key={a} style={{ padding: "0.65rem 1rem", display: "flex", alignItems: "center", gap: "0.6rem" }}>
              <Tick color={TEAL} />
              <span style={{ fontSize: "0.9rem", color: "#ffffff" }}>{a}</span>
            </Card>
          ))}
...
function S9() {
  ...
  return (
    <div style={{ width: "100%", maxWidth: "920px", display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
        {channels.map((c) => (
          <Card key={c.title} accent={c.color} style={{ padding: "1.5rem 1.5rem 1.75rem", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: c.color }} />
            <div style={{ width: 42, height: 42, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.4rem", background: `${c.color}18`, flexShrink: 0, marginBottom: "1rem" }}>{c.icon}</div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700, fontSize: "1.05rem", color: c.color, lineHeight: 1.3, minHeight: "2.75rem", display: "flex", alignItems: "flex-start", marginBottom: "0.5rem" }}>{c.title}</div>
        ...
        {phases.map((p, i) => (
          <Card key={p.horizon} accent={p.color} style={{ padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1rem", position: "relative", overflow: "hidden" }}>
            ...
            <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {p.items.map((item) => (
                <li key={item} style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", fontSize: "0.9rem", color: "#ffffff", lineHeight: 1.5 }}>
